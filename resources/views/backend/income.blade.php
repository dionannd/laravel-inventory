...
<div id="ribbon">
    ...
    <ol class="breadcrumb">
        <li>Home</li><li>Master Keuangan</li><li>Pemasukan</li>
    </ol>
</div>
<!-- END HEADER -->			
<!-- MAIN CONTENT -->
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-5 col-md-5 col-lg-8">
            <ul id="sparks" class="">
                ...
                <li class="sparks-info">
                    <a href="javascript:void(0);" class="btn btn-primary btn-flat" id="create"><i class="fa fa-plus"></i> Tambah Pemasukan</a>
                </li>
            </ul>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('body').on('click', '.edit', function(){
            var id = $(this).data('id');
            $.get('{{ route('income.index') }}'+'/'+ id +'/edit', function(data){
                $('#modal-header').html('Edit Pemasukan');
                $('#save').val('update');
                $('#modal').modal('show');
                $('#id').val(data.id)
                $('#expense').val(data.expense);
                $('#finance_id').val(data.finance_id);
                $('#desc').val(data.desc);
